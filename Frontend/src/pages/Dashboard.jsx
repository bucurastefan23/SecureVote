...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
        ...
        const res = await axios.get('http://localhost:5000/api/vote', {
            headers: { Authorization: `Bearer ${token}` }
        });
        setElections(res.data || []);

        const famRes = await axios.get('http://localhost:5000/api/family/my', {
            headers: { Authorization: `Bearer ${token}` }
        });
        if (famRes.data.family) {
            ...
            const invRes = await axios.get('http://localhost:5000/api/family/invitations', {
                headers: { Authorization: `Bearer ${token}` }
            });
  ...
  const handleCreateFamily = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.post('http://localhost:5000/api/family/create', { name: newFamilyName }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleInvite = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:5000/api/family/invite', { email: inviteEmail }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleRemoveMember = async (userId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:5000/api/family/remove-member', { userId }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleLeaveFamily = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:5000/api/family/leave', {}, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleAcceptInvite = async (inviteId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`http://localhost:5000/api/family/invitations/${inviteId}/accept`, {}, { headers: { Authorization: `Bearer ${token}` } });
      fetchData();
    } catch (err) {
  ...
  const handleRejectInvite = async (inviteId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`http://localhost:5000/api/family/invitations/${inviteId}/reject`, {}, { headers: { Authorization: `Bearer ${token}` } });
